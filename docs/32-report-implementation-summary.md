...
- NeoForge now has optional Reliquary Void Tear compatibility scoped to automatic collection in `fillTear`: Reliquary keeps its target discovery, matching locked stacks remain untouched, matching unlocked stacks remain consumable, and the stored increase is capped to that unlocked consumption. The compatibility has no global guard bypass or cross-call state.
- NeoForge 现提供只作用于 `fillTear` 自动收集的可选 Reliquary 虚空之泪兼容：Reliquary 保持原目标检索，同类锁定 stack 不变，同类未锁 stack 仍可被消费，内部增长量受该未锁消费量限制。兼容不使用全局守卫旁路或跨调用状态。
